...
fs.readdirSync(path.join(__dirname, 'routes')).forEach(file => {
  const filename = file.replace(/\.js$/, ''); // return a String
  const RouterMap = require(`./routes/${filename}`);  // return a Object with different route
  Object.keys(RouterMap).forEach((path) => {          // 'path' is each KEY of a file
    app.use(`/${filename}${path}`, (req, res, next) => {
      ...
      router.post('/', cors, (req, res, next) => func(req, res, next));
      router.get('/', cors, (req, res, next) => func(req, res, next)); //modify with cors

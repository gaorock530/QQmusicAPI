...
const song = {
  ...
  '/url': async (req, res) => {
    ...
    while (!purl && count < 10) {
      ...
      try{
        ...
        if (domain === '' && result.req_0 && result.req_0.data && result.req_0.data.sip) {
  ...
  '/urls': async (req, res) => {
    ...
    const reqFun = async () => {
      ...
      if (result.req_0 && result.req_0.data.testfile2g) {
        isOk = true;
      }
    };
    ...
    // prevent no cookie play
    if (result.code == '500001') return res.send({
      result: 400,
      errMsg: '获取播放链接出错',
    })
